  ...
  return(
    <div onClick={()=>onClick(job)} style={{background:'#fff',borderRadius:14,border:'1.5px solid #e2e8f0',padding:'16px 18px',cursor:'pointer',transition:'all .2s',boxShadow:'0 2px 10px rgba(3,105,161,.06)'}} className="hover:shadow-card-hover hover:-translate-y-0.5">
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8}}>
            ...
            <div style={{display:'flex',gap:6,alignItems:'center',flexShrink:0}}>
              {job.jobStatus&&<JobStatusBadge status={job.jobStatus} label={isArchived&&parseInt(job.id.replace(/\D/g,''),10)%2===0?ARCHIVED_ALT_LABEL:undefined}/>}
              {!isArchived&&job.badge&&<span style={{background:'#0369a1',color:'#fff',padding:'3px 9px',borderRadius:12,fontSize:11,fontWeight:800,flexShrink:0}}>{job.badge}</span>}
            </div>
          </div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <span style={{fontSize:12,color:'#9ca3af'}}>{isArchived?'🗄 Archived Vacancy':`📂 ${job.category}`}</span>
