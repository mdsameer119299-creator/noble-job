...
const real = (v: unknown): v is string => isRealDisplayValue(v)
    ...
    <Link href={`/jobs/govt/${job.slug || job.id}`} style={{ textDecoration: "none", display: "block" }}>
      <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e2e8f0", padding: "16px 18px", transition: "all .2s", boxShadow: "0 2px 10px rgba(24,71,212,.05)" }}
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12 }}>
          <div style={{ width: 48, height: 48, borderRadius: 10, background: job.color || "#1847d4", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 13, flexShrink: 0, textAlign: "center", lineHeight: 1.1 }}>{real(job.short) ? job.short : String(job.org ?? "").slice(0, 3).toUpperCase()}</div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
          {[
            real(job.vacancies) ? `👤 ${job.vacancies} Vacancies` : "",
            real(job.location) ? `📍 ${job.location}` : "",
            real(job.lastDate) ? `📅 Last: ${job.lastDate}` : "",
            real(job.salary) ? `💰 ${job.salary}` : "",
          ]
            .filter(Boolean)
            .map((t, i) => (
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 12, color: "#9ca3af" }}>{real(job.qualification) ? `🎓 ${job.qualification}` : ""}</span>
