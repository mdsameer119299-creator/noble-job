...
import{displayValue,isActionableJob}from'@/lib/jobs/renderable'
...
export function WfhDetailModal({job,open,onClose}:WfhDetailModalProps){
  ...
  // Only an ACTIONABLE job (complete, genuine, open, real application route) can be applied to.
  const actionable=!sample&&isActionableJob(job,'wfh')
  return(
    <Modal open={open} onClose={onClose} maxWidth="680px">
      ...
      <div style={{padding:'24px 28px'}}>
        ...
        {sample?(<>
          ...
          <button type="button" disabled aria-disabled="true" style={{display:'block',width:'100%',background:'#e2e8f0',color:'#64748b',padding:'14px',borderRadius:12,fontWeight:900,fontSize:16,border:'none',cursor:'not-allowed',fontFamily:'Playfair Display,serif'}}>Applications are not open for this listing</button>
        </>):(<>
