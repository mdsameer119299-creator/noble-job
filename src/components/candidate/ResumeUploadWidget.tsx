 ...
 * immediately sees an AI ATS score, detected skills, tips and a profile-
 * completion meter — no account required — then is invited to create a free
 * account (save + apply internally) or subscribe to daily job alerts.
 *
    ...
    return (
      <div>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ color: '#6b7280', fontSize: 11.5, marginTop: 5 }}>Create a free account to save your resume and unlock the rest.</div>
        </div>
        ...
        {result.suggestions.length > 0 && (
          <ul style={{ margin: '0 0 16px', paddingLeft: 18, color: '#374151', fontSize: 12.8, lineHeight: 1.6 }}>
            {result.suggestions.slice(0, 3).map((s, i) => <li key={i}>{s}</li>)}
          </ul>
        )}

        {/* Primary conversion: create account (save + apply internally) */}
        ...
        {/* Secondary conversion: daily alerts */}
        <JobAlertCta title="Or get matching jobs daily" compact />
      </div>
    )
  }
    ...
    <div>
      ...
      >
        ...
        <div style={{ fontWeight: 800, color: '#0d1f4e', fontSize: 15 }}>
          {state === 'busy' ? 'Scoring your resume…' : 'Upload your resume'}
        </div>
        <div style={{ color: '#6b7280', fontSize: 12.5, marginTop: 4 }}>
          {fileName || 'PDF, DOC or DOCX · up to 5 MB · instant AI score, no signup'}
        </div>
