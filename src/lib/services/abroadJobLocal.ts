...
export interface AbroadJobFilters {
  q?: string; country?: string; category?: string; status?: JobStatus | "all"; page?: number; limit?: number
}

function filterAbroadWithCounts(jobs: AbroadJob[], filters: AbroadJobFilters) {
  const { q, country, category, status } = filters
  let pre = [...jobs]
  if (q) { const term = q.toLowerCase(); pre = pre.filter(j => j.title.toLowerCase().includes(term) || j.company.toLowerCase().includes(term)) }
  ...
  list = rotateJobOrder(sortByStatus(list), "abroad", 3)
  return { list, counts }
}
