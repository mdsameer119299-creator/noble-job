...
CREATE OR REPLACE FUNCTION handle_new_employer()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
  IF NEW.role = 'employer' THEN
    INSERT INTO employers (user_id, company_name, designation)
    VALUES (
      NEW.id,
      COALESCE((SELECT raw_user_meta_data->>'company_name' FROM auth.users WHERE id = NEW.id), 'My Company'),
      COALESCE((SELECT raw_user_meta_data->>'designation' FROM auth.users WHERE id = NEW.id), '')
    )
    ON CONFLICT (user_id) DO NOTHING;
    INSERT INTO employer_plans (employer_id, plan_type, jobs_limit)
    SELECT id, 'free', 3 FROM employers WHERE user_id = NEW.id
    ON CONFLICT (employer_id) DO NOTHING;
    INSERT INTO employer_settings (employer_id)
    SELECT id FROM employers WHERE user_id = NEW.id
    ON CONFLICT (employer_id) DO NOTHING;
  END IF;
  RETURN NEW;
END;
...
BEGIN
  IF NEW.role = 'candidate' THEN
    INSERT INTO candidates (user_id, first_name, last_name)
    VALUES (
      NEW.id,
      COALESCE((SELECT raw_user_meta_data->>'first_name' FROM auth.users WHERE id = NEW.id), ''),
      COALESCE((SELECT raw_user_meta_data->>'last_name'  FROM auth.users WHERE id = NEW.id), '')
    )
    ON CONFLICT (user_id) DO NOTHING;
  END IF;
  RETURN NEW;
